...
def test_release_commits_last_preview_exactly():
    ...
    preview = sel.drag_tracker.last_preview_bins
    ...
    assert sel.model.snapshot() == frozenset(preview)
...
def test_add_and_subtract_semantics_across_multiple_drags():
    ...
    preview_add = sel.drag_tracker.last_preview_bins
...
def test_integration_freeze_outside_axes_but_commit_on_release_outside():
    ...
    assert selA.model.snapshot() == frozenset(preview_before)
